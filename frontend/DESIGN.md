...
- **Aula / reproductor** (`/dashboard/aula/[moduleId]`, estudiante): vista de "aula" a **dos columnas**. Columna principal = panel que renderiza el contenido seleccionado **según su tipo** (`LessonVideo` con skin propio sobre YouTube/Vimeo, texto enriquecido para Tema, visor de archivo/enlace para Material, `StudentActivity` para Actividad). Columna lateral = tarjeta con pestañas **Temario** (lista de contenidos + barra de progreso + completar), **Notas** (sus calificaciones: nota por actividad + nota del módulo + observación del docente) y **Chat** (mensajería en tiempo real con el/los docente(s) del módulo; la pestaña muestra un **badge ámbar** con cuántos contactos tienen mensajes sin leer). La selección de contenido es estado de cliente. (La antigua pestaña "Apuntes"/`ContentNote` fue eliminada.)
- **Chat del módulo** (`module-chat.tsx`, estudiante y docente): mensajería **1 a 1** docente↔estudiante en tiempo real por WebSocket. Burbujas estilo chat (propias `bg-primary` a la derecha, ajenas `bg-muted` a la izquierda) con hora; compositor con textarea + botón de enviar circular. El **estudiante** elige docente con un selector de contactos arriba (panel angosto del aula); el **docente** ve la lista de estudiantes a la izquierda + la conversación a la derecha. Cada contacto con mensajes sin leer muestra su contador. Detalle técnico (hook en el padre, badge en vivo, `active`, token por prop) en AGENTS.md.

### Centro de notificaciones

- **Campanario** (`NotificationBell`, topbar, PROFESSOR/STUDENT): botón circular navy (`bg-blue-950`) con campana y **badge ámbar** de no leídas; popover (Base UI, ver AGENTS.md) con las 8 recientes + "Ver todas". **Se actualiza en tiempo real por WebSocket** (`/notifications`, eventos `notification:new`/`notification:read`) — **sin polling**, así que las notificaciones nuevas (incl. mensajes de chat) y el conteo aparecen solos. **Bandeja** (`/dashboard/notificaciones`, estilo Gmail). **Al hacer clic en una notificación se abre en un modal** (`NotificationDetailDialog`, compartido por bandeja y campanario), **no** en una ruta de panel (no existe `/dashboard/notificaciones/[id]`); abrir = marcar leída. Icono por tipo = sobre `Mail` con **tinte distinto por tipo** (inscripción sky, asignación violet, aviso amber, calificación emerald, nueva actividad violet, **mensaje de chat `MessageSquare` blue**); el cuerpo resalta en **negrita** lo que va entre `«…»` (`NotificationBody`). Las notificaciones de **chat** (`MESSAGE`) y de **nueva actividad** (`ACTIVITY_PUBLISHED`) muestran un **botón de acción** ("Ir al chat" / "Ir a la actividad", `lib/notification-action.ts`) que lleva al módulo y marca la notificación leída. Tipos/tintes en `src/lib/notifications-meta.ts`. El **admin** redacta avisos en un modal "Nuevo aviso" (`/dashboard/notificaciones/enviar`, que muestra el historial).
...
- **Gestión de módulo** (`/dashboard/modulos/[moduleId]`, docente): `ModuleWorkspace` con **tres pestañas** (la pestaña **activa resalta con `bg-primary`**); el **admin no ve la pestaña Chat**:
  ...
  - **Calificaciones** (`Gradebook`): tabla **estudiantes × actividades** + columna de **nota del módulo** (calculada) + **observación** por estudiante (abre en **modal**, guarda en `ModuleGrade.observations`, no afecta la nota calculada) + un **botón de WhatsApp verde** por fila. **Encabezados ordenables** (por nombre, por cada actividad y por la columna Módulo; las notas vacías van al final). Botón **"+ Nueva calificación"** (modal: nombre, puntaje máx., peso) crea una **actividad presencial** (interna `isOffline`, sin entrega del estudiante): aparece como columna donde el docente **escribe el puntaje de cada estudiante directo en la celda**; pondera en la nota igual que cualquier actividad. Cada columna así creada se puede **editar (icono lápiz → modal; solo nombre y peso, no el puntaje máximo)** o eliminar. Estas actividades **no aparecen en el temario del aula**, solo en las "Notas" del estudiante.
  - **Chat** (`ModuleChat`): mensajería con los estudiantes inscritos (ver "Chat del módulo" arriba).
- **Calificación de una actividad con entrega** (`/dashboard/actividades/[activityId]`, docente): lista de estudiantes inscritos con su entrega (texto/archivo descargable), campo **nota /máximo** + retroalimentación por estudiante. Al **Guardar** se abre un **modal de confirmación**: **"Enviar y guardar"** (verde, WhatsApp) guarda **y** abre WhatsApp con la nota + retroalimentación prellenadas al número del estudiante, o **"No enviar, solo guardar"**. Guardar recalcula la nota ponderada del módulo.
- **Estado del módulo** (panel admin, `/dashboard/cursos/[id]`): cada módulo tiene un **control segmentado Activo / Concluido / Borrador** (`ModuleStatusControl`) que guarda al instante; el **estado activo se resalta por color** (Activo=emerald, Concluido=sky, Borrador=amber). Los módulos nuevos nacen **Activo**; un módulo en **Borrador no es visible para el estudiante** (ni en sidebar, detalle, aula o kárdex). **El detalle del programa** (`cursos/[id]`) divide su contenido en **dos pestañas grandes tipo tarjeta** (mitad y mitad): **"Módulos y docentes"** y **"Estudiantes inscritos"** (`CourseDetailTabs`). Asignar docentes a un módulo (`ModuleTeachersControl`) e inscribir estudiantes (`EnrollmentControl`) abren en **modal con buscador por nombre/apellido**; cada estudiante inscrito muestra un **botón de WhatsApp**.
...
**Crear/editar abren en un `Dialog`** (`user-dialogs.tsx` → `UserForm variant="dialog"`), no en páginas aparte. El formulario (`user-form.tsx` + `user-schema.ts` que espeja el DTO) pide nombre, apellido, correo, **teléfono (obligatorio)**, **documento de identidad (obligatorio y único)** y contraseña (toggle mostrar/ocultar, **mín. 6**); **no** tiene selector de Rol (el rol lo fija la sección al crear y se conserva al editar). El enlace "Volver …" y el botón Cancelar regresan a la sección de origen. **Carga masiva de estudiantes**: botón "Carga masiva" (solo sección estudiantes) → modal para **descargar plantilla `.xlsx`** y subir el archivo lleno; muestra el resumen "X de Y registrados" + filas con error. Los **409** por correo o documento duplicado se muestran inline en el campo correspondiente + banner + toast, nunca crashean.
