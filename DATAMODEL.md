...
| `QuestionType`      | `SINGLE_CHOICE \| MULTIPLE_CHOICE \| TRUE_FALSE \| SHORT_TEXT \| ESSAY \| FILE` | `Question.type` (motor de QUIZ/EXAM; `FILE` = entrega documental manual)                                                |
| `QuizAttemptStatus` | `IN_PROGRESS \| SUBMITTED \| GRADED`                                            | `QuizAttempt.status` (`SUBMITTED` = respuestas manuales pendientes de corregir)                                         |
| `RecoveryStage`     | `RECUPERATORIO \| SEGUNDA_INSTANCIA`                                            | `ModuleContent.recoveryStage` — examen de recuperación que puede elevar la nota hasta `Course.passingScore` (ver abajo) |
...
- **Excepción: el examen de recuperación (`ModuleContent.recoveryStage`).** Sobre un módulo **Concluido** con reprobados, el docente puede habilitar **un** examen `RECUPERATORIO` (tipo QUIZ/EXAM del motor de preguntas) y —si alguien lo rinde y sigue reprobado— **solo el admin** puede habilitar la `SEGUNDA_INSTANCIA`. Estos contenidos son la excepción al bloqueo del módulo Concluido (crear/editar/borrar, armar banco, rendir, autoguardar y corregir respuestas manuales); solo los ven/rinden los estudiantes **elegibles** y su publicación notifica solo a ellos. **No ponderan**: `recomputeModuleGrade` toma la mayor entre la nota ponderada, recuperatorio y segunda instancia, pero cada nota de recuperación se topa en `Course.passingScore`; nunca reduce una nota previa ni permite superar el mínimo por recuperación. Borrar el examen restaura el cálculo con las actividades restantes.
...
| `recoveryStage`           | `RecoveryStage?`    | **QUIZ/EXAM en módulo Concluido**: recuperatorio (docente/admin) o segunda instancia (solo admin, requiere recuperatorio). Máx. uno por etapa. No pondera: aporta `min(notaRecuperación, passingScore)` y se conserva la mayor entre nota ponderada y ambas etapas. Visible solo para elegibles. `null` = actividad normal |
| `createdAt` / `updatedAt` | `DateTime`          |                                                                                                                                                                                                                                                                                                                            |
...
| `type`            | `QuestionType`     | `SINGLE_CHOICE \| MULTIPLE_CHOICE \| TRUE_FALSE \| SHORT_TEXT \| ESSAY \| FILE`    |
...
| `manualScore`   | `Decimal(6,2)?`     | puntos sumados por el docente en ensayos/archivos |
...
Respuesta de un estudiante a una pregunta dentro de un intento. Además de `selectedOptionIds` (choice), `boolValue` (V/F) y `textValue` (corta/ensayo), una respuesta `FILE` guarda `fileUrl` (ruta `/files/submissions/...`), `fileName` y `fileSize`. `isCorrect`/`pointsAwarded` quedan `null` en `ESSAY`/`FILE` hasta la corrección manual. Índices: `@@unique([attemptId, questionId])`, `@@index([questionId])`.

**Motor de quiz (módulo `backend/src/quiz/`, `/me/quiz`, `JwtAuthGuard`):** docente — editor, banco, intentos y corrección manual; estudiante — runner, inicio, autoguardado y envío. Choice/V-F/short-text se autocalifican; `ESSAY` y `FILE` dejan el intento `SUBMITTED` hasta que el docente los puntúa. El archivo se sube antes por `/me/uploads` (máx. 40 MB) y se autoguarda con la respuesta; reemplazos y borrados limpian el blob best-effort. Al cerrar, `QuizService` llama `GradingService.applyQuizResult` para escribir `Submission.score` y recalcular `ModuleGrade`. `DELETE /me/quiz/attempts/:attemptId` permite al docente asignado o ADMIN reiniciar a ese estudiante: elimina todos sus `QuizAttempt` de la actividad (y sus `QuizAnswer` por cascada), borra la `Submission` derivada, recalcula la nota del módulo y limpia adjuntos `FILE`. Si el objetivo es un recuperatorio y ya existe un intento de segunda instancia, responde 409 hasta borrar primero esa etapa dependiente.
